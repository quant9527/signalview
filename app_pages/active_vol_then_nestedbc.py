"""active_vol_then_nestedbc page (url_path=active_vol_then_nestedbc)."""
...
TARGET_SIGNAL_PREFIX = "active_vol_then_nestedbc"
DISPLAY_SIGNAL_NAME = "active_vol_then_nestedbc"


def page_active_vol_then_nestedbc() -> None:
    """active_vol_then_nestedbc page (url_path=active_vol_then_nestedbc)."""
    st.set_page_config(layout="wide", page_title="active_vol_then_nestedbc")
    st.header("💰 active_vol_then_nestedbc")
    st.markdown("""
**模式特征：5分钟/15分钟级别放量信号 → 回调缩量 → 底部确认**

- 🎯 **核心逻辑**：小周期放量突破后的回调是绝佳买点
- 📊 **筛选条件**：5m 或 15m 出现 active_vol_then_nestedbc 信号
- ⏰ **最佳时机**：放量后缩量回调，出现底部确认信号时介入
""")
    st.divider()

    df_full = get_cached_data(45)
    df = df_full[df_full["signal_name"].str.startswith(
        "active_vol_then_nestedbc", na=False,
    )].copy()
    ...
    target_freqs = list(ACTIVE_VOL_NESTED_SHORT_FREQS)

    with st.expander(
        f"📋 信号配置 | 信号: {DISPLAY_SIGNAL_NAME}* | 周期: {', '.join(target_freqs)}",
        expanded=False,
    ):
    ...
    st.subheader("1️⃣ 目标标的（5m/15m active_vol_then_nestedbc 系列）")

    freq_lower = (
        df['freq'].astype(str).str.strip().str.lower()
        if 'freq' in df.columns else pd.Series(dtype=str)
    )
    ...
    if target_all_count > 0 and filtered_df.empty and 'freq' in df.columns:
        ...
        st.info(
            f"💡 本页只显示 **5m / 15m** 的 active_vol_then_nestedbc。"
            f"库中共 {target_all_count} 条，但周期多为：{sample_freqs}，"
            f"故当前筛选结果为 0。可放宽侧栏时间范围或确认数据源是否产出 5m/15m。"
        )

    if filtered_df.empty:
        st.warning(
            f"未找到 {DISPLAY_SIGNAL_NAME}* 信号"
            f"（周期：{', '.join(target_freqs)}）。侧栏时间范围内若有该信号"
            "但多为其他周期，此处会为空。"
        )
        st.stop()
    ...
    with col3:
        ...
        st.metric("⏱️ 5m信号数", int(freq_5m_count))
    with col4:
        freq_15m_count = (
            filtered_df['freq'].astype(str).str.lower() == '15m'
        ).sum()
        st.metric("⏱️ 15m信号数", int(freq_15m_count))
