"""nested_bc page (url_path=nested_bc)."""
...
def page_nested_bc() -> None:
    """nested_2bc weekly/daily page (url_path=nested_bc)."""
    st.set_page_config(layout="wide", page_title="nested_bc")
...
""")
    ...
    df = df_full[
        df_full["signal_name"].str.startswith(NESTED_2BC_PREFIX, na=False)
    ].copy()
    ...
    if target_all_count > 0 and filtered_df.empty and "freq" in df.columns:
        ...
        st.info(
            f"💡 本页只显示 **{' / '.join(target_freqs)}** 的 nested_2bc。"
            f"库中该前缀共 {target_all_count} 条，"
    ...
    with st.expander(
        f"📌 匹配到 {len(matched_signals)} 种 signal_name", expanded=False,
    ):
