"""Signal overview page (url_path=overview)."""
...
def page_overview() -> None:
    """Signal overview (url_path=overview)."""
    st.set_page_config(layout="wide", page_title="Signal Overview")
    ...
    signal_date_counts = df_recent.groupby(
        ['signal_name', df_recent['signal_date'].dt.date]
    ).size().reset_index(name='count')
    signal_date_counts.columns = ['signal_name', 'signal_date', 'count']

    pivot_table = signal_date_counts.pivot(
        index='signal_name', columns='signal_date', values='count'
    ).fillna(0).astype(int)
    ...
    date_cols = [
        ...
        or isinstance(col, type(pivot_table.columns[0]))
    ]
    ...
    for i in range(len(date_cols) - 1):
        ...
        def calc_change(row, current_date=current_date, previous_date=previous_date):
    ...
    pivot_table['Total'] = pivot_table[[col for col in date_cols]].apply(
        lambda row: sum([int(str(v).split()[0]) for v in row]),
        axis=1,
    )
