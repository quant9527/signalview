...
def render_dashboard(exchange_type: str):
    """Render dashboard for a specific exchange type."""
    ...
    # Page title with exchange type
    st.title(f"📊 {exchange_type.upper()} Dashboard")
    ...
    # Use the passed exchange type
    dashboard_type = exchange_type
    ...
    with col1:
        ...
        time_options = ["Last 24 hours", "Last 7 days", "Last 30 days", "Last 90 days", "Custom"]
        
        # Set default value based on exchange type
        # Binance defaults to "Last 24 hours", others default to "Last 30 days"
        time_key = f"time_range_{dashboard_type}"
        default_time_value = "Last 24 hours" if dashboard_type == "binance" else "Last 30 days"
        
        # Initialize session state with default if not set
        if time_key not in st.session_state:
            st.session_state[time_key] = default_time_value
    ...
    if time_range == "Last 24 hours":
        ...
        with col_start:
            start_date = st.date_input("Start Date", value=datetime.now() - timedelta(days=30), key=f"start_date_{dashboard_type}")
        with col_end:
            end_date = st.date_input("End Date", value=datetime.now(), key=f"end_date_{dashboard_type}")
    ...
    if hours:
        ...
        filtered_df = filtered_df[pd.to_datetime(filtered_df["signal_date"]) >= cutoff_time]
    elif days:
    ...
    with stat_col3:
        if "signal_date" in filtered_df.columns and len(filtered_df) > 0:
            min_date = pd.to_datetime(filtered_df["signal_date"]).min()
            max_date = pd.to_datetime(filtered_df["signal_date"]).max()
    ...
    view_mode = st.radio(
        ...
        index=0,
        horizontal=True,
        key=f"view_mode_{dashboard_type}"
    )
    ...
    if not filtered_df.empty:
        if view_mode == "Detail":
            ...
            # Create combined symbol display for compact view too
            if 'symbol' in filtered_df_filled.columns and 'symbol_name' in filtered_df_filled.columns:
                filtered_df_filled['_display_symbol'] = filtered_df_filled.apply(
                    lambda row: f"{row['symbol']} - {row['symbol_name']}", axis=1
                )
            ...
            # Reorder columns to prioritize display symbol
            if '_display_symbol' in filtered_df_filled.columns:
                column_order = ['exchange', '_display_symbol', 'signal_name', 'count', 'signal_info']
                # Merge the display symbol back into grouped df
                grouped_df = grouped_df.merge(
                    filtered_df_filled[['exchange', 'symbol', 'signal_name', '_display_symbol']].drop_duplicates(),
                    on=['exchange', 'symbol', 'signal_name'],
                    how='left'
                )
            else:
        ...
        st.download_button(
            "📥 Export Data to CSV",
            csv,
            file_name=f"dashboard_{dashboard_type}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
            mime="text/csv",
            key=f"export_{dashboard_type}"
        )
    else:
