...
# First section: Exchange and Time Range (same column/group)
...
with col1:
    if st.button("AS Exchange", use_container_width=True, type="primary" if dashboard_type == 'as' else "secondary"):
...
with col2:
    if st.button("BINANCE Exchange", use_container_width=True, type="primary" if dashboard_type == 'binance' else "secondary"):
...
with col3:
    if st.button("EM Exchange", use_container_width=True, type="primary" if dashboard_type == 'em' else "secondary"):
...
# Time Range selection (right after exchange)
st.markdown("**Select Time Range:**")
time_range = st.selectbox(
    "Time Range",
    options=["Last 7 days", "Last 30 days", "Last 90 days", "Custom"],
...
# Second section: Signal Names selection
