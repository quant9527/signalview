...
available_freqs_all = df[df['signal_name'].str.startswith(TARGET_SIGNAL_PREFIX, na=False)]['freq'].unique().tolist() if 'freq' in df.columns else []

# 紧凑显示配置信息
with st.expander(f"📋 信号配置 | 信号: {', '.join(sorted(all_matching_signals)) or 'N/A'} | 周期: {', '.join(sorted(available_freqs_all)) or 'N/A'}", expanded=False):
    col1, col2 = st.columns(2)
    with col1:
        st.markdown(f"**目标信号（{TARGET_SIGNAL_PREFIX} 系列）**")
        st.text(', '.join(sorted(all_matching_signals)) if all_matching_signals else "暂无匹配信号")
    with col2:
        st.markdown("**可用周期**")
...
if not filtered_df.empty and 'signal_date' in filtered_df.columns:
    ...
    if 'freq' in filtered_df.columns:
        ...
        daily_signals = daily_signals.set_index('date').sort_index(ascending=False)
