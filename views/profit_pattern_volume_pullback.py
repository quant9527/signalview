...
# 
# 模式说明：
# 筛选5m和15m出现nested_2bc信号的标的，等待回调见底机会
# ============================================================================

st.header("💰 盈利模式2: 近期爆量-回调见底")
st.markdown("""
**模式特征：5分钟/15分钟级别爆量信号 → 回调缩量 → 底部确认**

- 🎯 **核心逻辑**：小周期放量突破后的回调是绝佳买点
- 📊 **筛选条件**：5m 或 15m 出现 nested_2bc 信号
- ⏰ **最佳时机**：爆量后缩量回调，出现底部确认信号时介入
""")
...
# 配置区域（紧凑显示）
# ============================================================================
TARGET_SIGNAL_PREFIX = 'nested_2bc'
TARGET_FREQS = ['5m', '15m']

# 紧凑显示配置信息
with st.expander(f"📋 信号配置 | 信号: {TARGET_SIGNAL_PREFIX}* | 周期: {', '.join(TARGET_FREQS)}", expanded=False):
...
# ============================================================================
# 筛选 5m 和 15m 的 nested_2bc 系列信号
# ============================================================================
st.subheader("1️⃣ 近期爆量标的（5m/15m nested_2bc 系列）")

# 筛选条件：signal_name 以 nested_2bc 开头 且 freq in (5m, 15m)
filtered_df = df[
    (df['signal_name'].str.startswith(TARGET_SIGNAL_PREFIX, na=False)) & 
    (df['freq'].isin(TARGET_FREQS))
].copy()

if filtered_df.empty:
    st.warning(f"未找到 {TARGET_SIGNAL_PREFIX}* 信号（周期：{', '.join(TARGET_FREQS)}）")
    st.stop()
...
with col3:
    freq_5m_count = len(filtered_df[filtered_df['freq'] == '5m'])
    st.metric("⏱️ 5m信号数", freq_5m_count)
with col4:
    freq_15m_count = len(filtered_df[filtered_df['freq'] == '15m'])
...
st.download_button(
    "📥 导出数据",
    csv,
    file_name=f"profit_pattern_volume_pullback_{pd.Timestamp.now().strftime('%Y%m%d_%H%M%S')}.csv",
    mime="text/csv"
)
